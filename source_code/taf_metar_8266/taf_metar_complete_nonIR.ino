/*  B7971 DUSTOFF IoT BOARD non IR
 ...
 *  Additional modules to be added in later updates, to include sports scores, stock data, etc. 
...
/* WiFi */
// Your SSID and PW, adjust as required
...
// Your current location; adjust as required. An update in the future will allow automatic triangulation of the 8266 location using LocationAPI from Unwired Labs.
...
/* Configuration */
int pollDelay = 20000; //time between each retrieval of TAF/METAR data
...
void setup() {
  ...
  // Scrolling option through scroll function provided below. However, if static display requested:
  // Retain "$B7M" and "<cr>, but replace "MEDEVAC" as desired for start-up display, for example:
...
void connectToWifi() {
  scroll("connecting to WIFI...");
  //connect to wifi
  // Serial.print("connecting to ");
  // Serial.println(ssid);
  WiFi.mode(WIFI_STA);
  WiFi.begin(ssid, password);
  while (WiFi.status() != WL_CONNECTED) { //WAITING FOR WIFIg
    delay(500);
    // Serial.print(".");
  }
  // Serial.println("");
...
void loop() {
  // initates call for METAR and TAF data 
  if (awaitingArrivals) {
